...
# # Format nama file berdasarkan variabel tanggal
# file_class_name = f"print_{current_day}_{current_month_eng}_{current_year}_class.csv"
# file_name = f"print_{current_day}_{current_month_eng}_{current_year}.csv"

# # Format path S3
# s3_class_path = f"s3://la-s3-flight/{current_month_id.capitalize()} {current_year}/{file_class_name}"
# s3_path = f"s3://la-s3-flight/{current_month_id.capitalize()} {current_year}/{file_name}"
# Format path S3
s3_class_path = f"s3://lp-s3-dwh/testing_dew/lion_air/print_sep_class_2024.csv"
s3_path = f"s3://lp-s3-dwh/testing_dew/lion_air/print_sep_2024.csv"

# # Fungsi untuk log informasi S3 path
# def log_s3_path():
#     logger.info(f"Path S3 untuk file flight class: {s3_class_path}")
#     logger.info(f"Path S3 untuk file flight summary: {s3_path}")
...
with DAG(
    ...
    # # Task untuk logging path S3
    # log_s3_task = PythonOperator(
    #     task_id='log_s3_task',
    #     python_callable=log_s3_path
    # )
    ...
    execute_redshift_sql = RedshiftDataOperator(
        ...
        database='andreas',  # Database Redshift
        db_user='admin_andreas',  # User Redshift ************ Ganti dengan user Redshift Anda
        sql="""
        ...
        iam_role 'arn:aws:iam::888888888:role/andreas_dev'
